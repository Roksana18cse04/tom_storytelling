...
class LLMService:
    ...
    async def generate_followup(self, user_id: str, session_id: str, user_input: str) -> str:
        """
        Generate a context-aware follow-up question based on user's latest response and story history.
        """
        ...
        prompt = f"""
        You are a compassionate interviewer continuing a personal life story conversation.
        Current phase: {current_phase}.
        Conversation so far:
        {history_text}
        ...
        Here are some example questions you might take inspiration from:
        {sample_questions}

        Based on the story so far, generate ONE natural, emotionally intelligent follow-up question
        to help the user elaborate further. You may adapt or invent new questions inspired by these examples.
        ...
        response = await client.chat.completions.create(
            model=self.model,
            messages=[
                {"role": "system", "content": "You are an empathetic interviewer helping users tell their stories."},
                {"role": "user", "content": prompt},
            ],
